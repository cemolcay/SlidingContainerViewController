...
class SlidingContainerSliderView: UIScrollView, UIScrollViewDelegate {
    ...
    func draw () {
        ...
        var labelTag = 0
        var currentX = appearance.padding
        for title in titles {
            ...
            addSubview(label)
            labels.append(label)
            currentX += label.frame.size.width + appearance.padding
        }
    ...
    func selectItemAtIndex (index: Int) {
        ...
        for i in 0..<self.labels.count {
            ...
            if i == index {
                ...
                let selectorPad: CGFloat = 10
                UIView.animateWithDuration(0.3, animations: {
                    [unowned self] in
                    self.selector.frame = CGRect (
                        x: label.frame.origin.x - selectorPad,
                        y: self.selector.frame.origin.y,
                        width: label.frame.size.width + selectorPad*2,
